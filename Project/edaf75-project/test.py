...
def check_all():
    try:
        ...
        for ingredient_name, unit in INGREDIENT_INPUT:
            ...
            resource = url("/storages")
            r = requests.post(resource, json=payload)
            require(r.status_code, 201, "Wrong status code from POST on /storages")
            location = r.json()['location']
        ...
        for ingredient_name, quantity in INGREDIENT_DELIVERY_INPUT:
            ...
            r = requests.post(url(f"/storages/{quote(ingredient_name)}/deliveries"), json=payload)
        ...
        cook = []
        for val in found_cookies:
            for subval in val:
                cook.append(subval)
        require_same_values(cook, actual_cookies, f"GET /cookies returned the wrong cookies")
        ok()
        ...
        r = requests.post(url(f"/storages/{quote('Butter')}/deliveries"), json=payload)
        require(r.status_code, 201, "Wrong status code from POST on /ingredients/.../deliveries")
        ...
        r = requests.post(url(f"/storages/{quote('Flour')}/deliveries"), json=payload)
        require(r.status_code, 201, "Wrong status code from POST on /storages/.../deliveries")
